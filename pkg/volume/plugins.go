...
func (pm *VolumePluginMgr) InitPlugins(plugins []VolumePlugin, prober DynamicPluginProber, host VolumeHost) error {
	...
	for _, plugin := range plugins {
		// 特别标注 如果是CSI 则这个地方name="kubernetes.io/csi"
...
func (pm *VolumePluginMgr) FindPluginByName(name string) (VolumePlugin, error) {
	...
	// Issue warning if the matched provider is deprecated
	// 判断是不是内置提供的卷 如果是要出error
	if detail, ok := deprecatedVolumeProviders[matches[0].GetPluginName()]; ok {
		klog.Warningf("WARNING: %s built-in volume provider is now deprecated. %s", matches[0].GetPluginName(), detail)
	}
	// 注: 匹配到CSI插件 CSIPluginName = "kubernetes.io/csi"
	return matches[0], nil
}
...
func (pm *VolumePluginMgr) FindAttachablePluginBySpec(spec *Spec) (AttachableVolumePlugin, error) {
	...
	// 有的插件不需要attach/deattch 实现AttachableVolumePlugin接口的才可以attach/detach
...
func (pm *VolumePluginMgr) FindNodeExpandablePluginByName(name string) (NodeExpandableVolumePlugin, error) {
	// NodeExpandableVolumePlugin 可以对数据卷进行扩容操作
	volumePlugin, err := pm.FindPluginByName(name)
	if err != nil {
		return nil, err
	}
