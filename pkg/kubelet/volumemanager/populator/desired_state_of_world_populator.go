...
func (dswp *desiredStateOfWorldPopulator) populatorLoop() {
	// 先获取当前节点中所有被添加的新Pod，然后遍历Pod根据当前状态修改为期望状态
	...
	// 先获取当前节点中所有被删除的Pod，然后遍历Pod根据当前状态修改为期望状态
...
func (dswp *desiredStateOfWorldPopulator) findAndAddNewPods() {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.ExpandInUsePersistentVolumes) {
		...
		for _, mountedVolume := range dswp.actualStateOfWorld.GetMountedVolumes() {
			...
			// 把pod-->volume添加到mountedVolumesForPod
			if !exist {
	...
	// 从podManager里获取pod 并遍历调用processPodVolumes
...
func (dswp *desiredStateOfWorldPopulator) processPodVolumes(
	...
	// 获取pod的volumeMount和volumeDevices
	...
	for _, podVolume := range pod.Spec.Volumes {
		// 如果pod.spec.volume不在mount/devices里则忽略
