...
func (og *operationGenerator) GenerateRegisterPluginFunc(
	...
	registerPluginFunc := func() error {
		...
		defer cancel()
		// 调用node-driver-registerar.GetInfo 获取CSI插件信息
		...
		// 判断插件类型 默认Type=CSIPlugin类型
		handler, ok := pluginHandlers[infoResp.Type]
		if !ok {
			// 通知node-driver-registerar 注册失败 没有此插件类型对应的插件处理程序
