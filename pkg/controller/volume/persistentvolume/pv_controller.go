...
func (ctrl *PersistentVolumeController) syncClaim(claim *v1.PersistentVolumeClaim) error {
	...
	claim = newClaim
	// 根据"pv.kubernetes.io/bind-completed"来判断pvc和pv是否绑定完成
...
func (ctrl *PersistentVolumeController) syncVolume(volume *v1.PersistentVolume) error {
	...
	if volume.Spec.ClaimRef == nil {
		...
		// 如果uid为空 则需要同步pv和pvc的绑定状态 暂时设置空闲状态
		...
		// 如果正在被bound中 则更新cache 并更新状态为bound
		...
		// 判断注解是否有pv.kubernetes.io/bound-by-controller
		// 如果找不到pvc和注解存在
		if !found && metav1.HasAnnotation(volume.ObjectMeta, pvutil.AnnBoundByController) {
			...
			// 如果pv状态不是Released和Failed 从infomer cache里找 如果找不到 从apiserver里获取
		...
		if claim == nil {
			...
			// claim to it.
			// 把pvc加入到pvc队列 等待pvcworker处理pvc与pv到绑定
			ctrl.claimQueue.Add(claimToClaimKey(claim))
			return nil
		} else if claim.Spec.VolumeName == volume.Name {
			// pv和pvc已经绑定 那么更新pv的状态为bound
...
func (ctrl *PersistentVolumeController) reclaimVolume(volume *v1.PersistentVolume) error {
	...
	// 回收策略 保留（Retain），回收（Recycle）和删除（Delete)
...
func (ctrl *PersistentVolumeController) recycleVolumeOperation(volume *v1.PersistentVolume) {
	...
	if err != nil {
		...
		// 如果找不到插件 则需要更新pv状态为failed 回收失败 并update event
...
func (ctrl *PersistentVolumeController) provisionClaim(claim *v1.PersistentVolumeClaim) error {
	...
	// 根据claim.Spec.StorageClassName或者注解"volume.beta.kubernetes.io/storage-class"查到插件及sotrageclass
...
func (ctrl *PersistentVolumeController) provisionClaimOperation(
	...
	// storageClass.Parameters 会记录rdb的详细信息 例如poll name secret
	...
	// 需要实现ProvisionableVolumePlugin接口才可以完成
...
func (ctrl *PersistentVolumeController) findProvisionablePlugin(claim *v1.PersistentVolumeClaim) (vol.ProvisionableVolumePlugin, *storage.StorageClass, error) {
	...
	// 根据claim.Spec.StorageClassName 根据注解"volume.beta.kubernetes.io/storage-class"
	claimClass := v1helper.GetPersistentVolumeClaimClass(claim)
	// 获取storageclass对象
	...
	// 根据class.Provisioner从in-tree里查找插件
