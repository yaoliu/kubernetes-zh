...
func (ctrl *PersistentVolumeController) updateVolume(volume *v1.PersistentVolume) {
	...
	// is an old version.
	// cache
	// 将volume更新到ctrl.volume.store中 如果不存在则添加进去
...
func (ctrl *PersistentVolumeController) deleteVolume(volume *v1.PersistentVolume) {
	...
	// 如果pv关联了pvc 那么还需要把pvc对应的key加入到队列 等claimsWorker处理
...
func (ctrl *PersistentVolumeController) Run(stopCh <-chan struct{}) {
	...
	// resync方法会周期性的运行 主要功能是使用infomer机制中Lister方法获取PV/PVC加入到工作队列中 等待volumeWorker,claimWorker消费处理
	go wait.Until(ctrl.resync, ctrl.resyncPeriod, stopCh)
	// 负责PV状态维护 从工作队列消费pv 对pv进行与PVC的绑定/解绑/删除等操作 并根据回收策略进行对数据处理
	go wait.Until(ctrl.volumeWorker, time.Second, stopCh)
	// 负责pvc的状态 从工作队列消费pvc 对pvc进行与pv对绑定/解绑/删除等操作
